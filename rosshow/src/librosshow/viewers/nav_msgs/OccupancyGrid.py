...
class OccupancyGrid(object):
    ...
    def draw(self):
        ...
        w = self.g.shape[0]
        h = self.g.shape[1]
        current_image = np.asarray(self.image.data, np.uint8).reshape((self.image.info.height, self.image.info.width))
        current_image = np.array((current_image.T, current_image.T, current_image.T)).T
        current_image = np.asarray(self.image.data, np.uint16).reshape((self.image.info.height, self.image.info.width)).astype(np.float)
